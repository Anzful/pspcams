...
MAX_CAMERAS = 4  # Set the maximum limit for cameras
    ...
    async def check_camera(camera):
        async with semaphore:
            ...
            if connected:
                try:
                    camera_ip = camera["ip"]
                    get_config_url = f"http://{camera_ip}/cgi-bin/configManager.cgi?action=getConfig&name=VideoInDayNight"
                    async with aiohttp.ClientSession() as session:
                        async with session.get(get_config_url, auth=aiohttp.BasicAuth(
                                camera["username"], camera["password"]), timeout=5) as response:
                            if response.status == 200:
                                if "BlackWhite" in await response.text():
                                    night_mode_count += 1
                                else:
                                    day_mode_count += 1
